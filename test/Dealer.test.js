...
describe('Dealer', function () {
    ...
    describe('newGame', function () {
        it('can create multiple game instances', async function () {
            ...
            game1 = await Game.at(await dealer.games(0));
            game2 = await Game.at(await dealer.games(1));
    ...
    describe('join', function () {
        beforeEach(async function () {
            ...
            game = await Game.at(await dealer.games(0));
    ...
    describe('bet', function () {
        ...
        it('reverts if player tries to jump the queue', async function () {
            await expectRevert(dealer.bet(0, ante, { from: p2 }), "player is not next in queue");
        });

        it('reverts if player bets less than ante', async function () {
            await expectRevert(dealer.bet(0, ante - 1, { from: p1 }), "bet must not be less than ante");
            await dealer.bet(0, ante, { from: p1 });
        });
        ...
        it('wins bet if final card is Inside', async function () {
            ...
            await dealer.bet(0, 300, { from: p3 });
        ...
        it('loses bet if final card is Outside', async function () {
            ...
            await dealer.topUp({ from: p3, value: 10 * ante });
            await dealer.join(0, { from: p3 });
            await dealer.bet(0, ante, { from: p3 });
        ...
        it('can only bet up to total pot', async function () {
            ...
            await dealer.topUp({ from: p3, value: 10 * ante });
            await dealer.join(0, { from: p3 });
            await dealer.bet(0, 500, { from: p3 });
